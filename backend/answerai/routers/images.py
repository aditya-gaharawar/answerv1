...
from answerai.env import ENABLE_FORWARD_USER_INFO_HEADERS, SRC_LOG_LEVELS
from answerai.routers.files import upload_file
from answerai.utils.auth import get_admin_user, get_verified_user
from answerai.utils.images.comfyui import (
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-OpenAnswerAI-User-Name"] = user.name
                headers["X-OpenAnswerAI-User-Id"] = user.id
                headers["X-OpenAnswerAI-User-Email"] = user.email
                headers["X-OpenAnswerAI-User-Role"] = user.role
