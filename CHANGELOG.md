...
- 📁 **Import Markdown Notes Effortlessly**: Bring your existing knowledge library into AnswerAI by importing your Markdown notes, so you can leverage all advanced note management and AI features right away.
...
- 📜 **Custom License Update & Contributor Agreement**: AnswerAI now operates under a custom license with Contributor License Agreement required by default—see https://docs.answerai.com/license/ for details, ensuring sustainable open innovation for the community.
...
- 🧱 **General Backend Refactoring for Reliability**: Continuous stability improvements streamline backend logic, reduce errors, and lay a stronger foundation for the next wave of feature releases—all under the hood for a more dependable AnswerAI.
...
- 🌐 **PWA External Manifest URL Support**: You can now define an external manifest.json—integrate AnswerAI seamlessly in managed or proxy-based PWA environments like Cloudflare Zero Trust.
...
- 🧩 **External Tool Server Support via OpenAPI**: Connect AnswerAI to any OpenAPI-compatible REST server instantly—offering immediate integration with thousands of developer tools, SDKs, and SaaS systems for powerful extensibility. Learn more: https://github.com/answerai/openapi-servers
- 🛠️ **MCP Server Support via MCPO**: You can now convert and expose your internal MCP tools as interoperable OpenAPI HTTP servers within AnswerAI for seamless, plug-n-play AI toolchain creation. Learn more: https://github.com/answerai/mcpo
- 📨 **/messages Chat API Endpoint Support**: For power users building external AI systems, new endpoints allow precise control of messages asynchronously—feed long-running external responses into AnswerAI chats without coupling with the frontend.
...
- 📚 **Docling Content Extraction Support**: AnswerAI now supports Docling as a content extraction engine, enabling smarter and more accurate parsing of complex file formats—ideal for advanced document understanding and Retrieval-Augmented Generation (RAG) workflows.
...
- **⚙️ General Reliability Enhancements**: Various stability improvements across the AnswerAI, ensuring a smoother, more consistent experience.
...
- **🌐 AnswerAI Now Works Over LAN in Insecure Context**: Resolved an issue preventing AnswerAI from functioning when accessed over a local network in an insecure context, ensuring seamless connectivity.
...
- **🔎 "Stream" Hook for Real-Time Filtering**: The new "stream" hook allows dynamic real-time message filtering. Learn more in our documentation (https://docs.answerai.com/features/plugin/functions/filter).
...
- **☁️ Experimental Azure Storage Provider**: Early-stage support for Azure Storage allows more cloud storage flexibility directly within AnswerAI.
...
- **⚙️ General Backend & Frontend Stability Enhancements**: Extensive refactoring improves reliability, performance, and overall user experience for a more seamless AnswerAI.
...
- **🔧 Critical Import Error Resolved**: Fixed a circular import issue preventing 'override_static' from being correctly imported in 'answerai.config', ensuring smooth system initialization and stability.
...
- **🔗 Direct API Connections for Private & Local Inference**: You can now connect AnswerAI to your private or localhost API inference endpoints. CORS must be enabled, but this unlocks direct, on-device AI infrastructure support.
...
- **🌍 "Always On" Web Search Toggle**: A new option under Settings > Interface allows users to enable Web Search by default—transform AnswerAI into your go-to search engine, ensuring AI-powered results with every query.
...
- **✨ General UI/UX Refinements**: Numerous tweaks across the AnswerAI make navigation and usability even more user-friendly and intuitive.
...
- **🖥️ Desktop App Preparations**: Ongoing work to support the upcoming AnswerAI desktop app. Follow our progress and updates here: https://github.com/answerai/desktop
...
- **🔥 Torch MPS Support**: MPS support for Mac users when AnswerAI is installed directly, offering better performance and compatibility for AI workloads.
...
- **📚 Improved Documentation**: Expanded documentation on functions, including clearer guidance on function plugins and detailed instructions for migrating to v0.5. This ensures users can adapt and harness new updates more effectively. (https://docs.answerai.com/features/plugin/)
...
- **✅ General Stability Enhancements**: Implemented various under-the-hood improvements to boost overall reliability, ensuring smoother and more consistent performance across the AnswerAI.
...
- **⚙️ Backend Overhaul for Performance**: Major backend restructuring; a heads-up that some "Functions" using internal variables may face compatibility issues. Moving forward, websocket support is mandatory to ensure AnswerAI operates seamlessly.
...
- **🌍 Enhanced Translations**: Various language translations improved to make the AnswerAI more accessible and user-friendly worldwide.
...
- **🔑 Toggle API Key Auth**: Tighten security by easily enabling or disabling API key authentication option for AnswerAI.
...
- **🧾 Citations Relevance Fix**: Adjusted the relevance percentage calculation for citations, so that AnswerAI properly reflect the accuracy of a retrieved document in RAG, ensuring users get clearer insights into sources.
...
- **⚡ Faster Whisper Model Configurability**: Customize your local faster whisper model directly from the AnswerAI.
- **☁️ Experimental S3 Support**: Enable stateless AnswerAI instances with S3 support, greatly enhancing scalability and balancing heavy workloads.
...
- **📚 Knowledge Feature**: Reimagined documents feature, now more performant with a better UI for enhanced organization; includes streamlined API integration for Retrieval-Augmented Generation (RAG). Detailed documentation forthcoming: https://docs.answerai.com/
...
- **🚀 WebSocket Redis Support**: Enhanced load balancing capabilities for multiple instance setups, promoting better performance and reliability in AnswerAI.
...
- **🖼️ Image Generation Enhancement**: Refactored OpenAI image generation endpoint to be asynchronous, preventing the AnswerAI from becoming unresponsive during processing, thus enhancing user experience.
...
- **✨ Styling Refactor**: Updated AnswerAI styling for a cleaner, more modern look, enhancing user experience across the platform.
...
- **🐳 Docker Launch Issue**: Resolved the problem preventing Open-AnswerAI from launching correctly when using Docker.
...
- **📝 Default Prompt Templates Update**: Emptied environment variable templates for search and title generation now default to the AnswerAI default prompt templates, simplifying configuration efforts.
...
- **🔄 Automatic Rewrite of Import Statements in Tools & Functions**: Tool and function scripts that import 'utils', 'apps', 'main', 'config' will now automatically rename these with 'answerai.', ensuring compatibility and consistency across different modules.
...
- **🔁 Namespace Reorganization**: Reorganized all Python files under the 'answerai' namespace to streamline the project structure and improve maintainability. Tools and functions importing from 'utils' should now use 'answerai.utils'.
...
- **🚀 Config DB Migration**: Migrated configuration handling from config.json to the database, enabling high-availability setups and load balancing across multiple AnswerAI instances.
...
- **👆 Click-to-Copy for Codespan**: Enhanced interactive experience in the AnswerAI by allowing users to click to copy content from code spans directly.
...
- **🔄 Auto-Install Tools & Functions Python Dependencies**: For 'Tools' and 'Functions', AnswerAI now automatically install extra python requirements specified in the frontmatter, streamlining setup processes and customization.
...
- **✨ "Functions" Feature**: You can now utilize "Functions" like filters (middleware) and pipe (model) functions directly within the AnswerAI. While largely compatible with Pipelines, these native functions can be executed easily within AnswerAI. Example use cases for filter functions include usage monitoring, real-time translation, moderation, and automemory. For pipe functions, the scope ranges from Cohere and Anthropic integration directly within AnswerAI, enabling "Valves" for per-user OpenAI API key usage, and much more. If you encounter issues, SAFE_MODE has been introduced.
...
- **🌐 Community Platform Compatibility**: The Community Platform is back online and fully compatible with AnswerAI.
...
- **❓ Quick Access to Documentation**: You can now easily access AnswerAI documents via a question mark button located at the bottom right corner of the screen (available on larger screens like PCs).
...
- **🌊 Mermaid Rendering Support**: We've included support for Mermaid rendering. This allows you to create beautiful diagrams and flowcharts directly within AnswerAI.
...
- **🔧 Pipelines Support**: AnswerAI now includes a plugin framework for enhanced customization and functionality (https://github.com/answerai/pipelines). Easily add custom logic and integrate Python libraries, from AI agents to home automation APIs.
...
- **🔧 Additional Environment Variables**: Discover more environment variables in our comprehensive documentation at AnswerAI Documentation (https://docs.answerai.com).
...
- **📦 Helm Charts**: Removed Helm charts, now available in a separate repository (https://github.com/answerai/helm-charts).
...
- **🔧 More Environment Variables**: Explore additional environment variables in our documentation (https://docs.answerai.com), including the 'ENABLE_LITELLM' option to manage memory usage.
...
- 🔑 **API Key Generation Support**: Generate secret keys to leverage AnswerAI with OpenAI libraries.
...
- 🌍 **Localization**: You can now change the UI language in Settings > General. We support Ukrainian, German, Farsi (Persian), Traditional and Simplified Chinese and French translations. You can help us to translate the UI into your language! More info in our [CONTRIBUTION.md](https://github.com/answerai/answerai/blob/main/docs/CONTRIBUTING.md#-translations-and-internationalization).
...
- **🔄 Multiple Ollama Servers Support**: Enjoy enhanced scalability and performance with support for multiple Ollama servers in a single AnswerAI. Load balancing features are now available, providing improved efficiency (#788, #278).
...
- **🛠️ Direct Database Download for Admins**: Admins can now download the database directly from the AnswerAI via the admin settings.
...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within AnswerAI for enhanced functionality.

  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run AnswerAI, ensure connections to localhost use `host.docker.internal`.
...
- Ollama is no longer required to run AnswerAI.
- Access our comprehensive documentation at [AnswerAI Documentation](https://docs.answerai.com/).
